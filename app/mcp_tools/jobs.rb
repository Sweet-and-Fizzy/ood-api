  ...
  module Tools
    ...
    class GetJobTool < MCP::Tool
      ...
      def self.call(server_context:, cluster_id:, job_id:, **_params)
        user = ENV['USER'] || ENV['LOGNAME'] || 'unknown'
        job, cluster = Handlers::Audit.log(op: 'get_job', user: user, source: 'mcp', cluster: cluster_id,
                                           job_id: job_id) do
          Handlers::Jobs.get(
            clusters:   OodApi::App.clusters,
            cluster_id: cluster_id,
    ...
    class ListHistoricJobsTool < MCP::Tool
      ...
      def self.call(server_context:, cluster_id:, **_params)
        user = ENV['USER'] || ENV['LOGNAME'] || 'unknown'
        jobs, _cluster = Handlers::Audit.log(op: 'list_historic_jobs', user: user, source: 'mcp',
                                             cluster: cluster_id) do
    ...
    class CancelJobTool < MCP::Tool
      ...
      def self.call(server_context:, cluster_id:, job_id:, **_params)
        user = ENV['USER'] || ENV['LOGNAME'] || 'unknown'
        result = Handlers::Audit.log(op: 'cancel_job', user: user, source: 'mcp', cluster: cluster_id,
                                     job_id: job_id) do
